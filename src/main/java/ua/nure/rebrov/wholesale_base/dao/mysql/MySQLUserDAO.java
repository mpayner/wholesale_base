...
public class MySQLUserDAO extends MySQLschema implements UserDAO {
    ...
    public List<User> getAdmins() {
        List<User> list = new LinkedList<>();
        try {
            PreparedStatement ps = con.prepareStatement("select u.id, u.name, u.email, u.password, u.phone_number, u.type_id, u.address, u.base_id,  b.name base_name,  b.address base_address from user u left join base b on u.base_id = b.id and u.type_id = 1");
            ResultSet rs = ps.executeQuery();
            while(rs.next()) {
                User user = new User();
                user.setName(rs.getString("name"));
